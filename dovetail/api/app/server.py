...
class Server(object):
    ...
    def set_vm_images(self):
        image_path = os.path.join(self.dovetail_home, str(self.requestId), 'images')
        try:
    ...
    def set_conf_files(self):
        config_path = os.path.join(self.dovetail_home, str(self.requestId), 'pre_config')
        ...
        for key, value in constants.CONFIG_YAML_FILES.items():
            ...
            try:
                utils.write_yaml_file(self.requestData['conf'][key], config_file)
            except KeyError:
                pass
            except Exception as e:
                return str(e), False
    ...
    def get_execution_status(self, testsuite, request_testcases, exec_testcases):
        results_dir = os.path.join(self.dovetail_home, str(self.requestId), 'results')
        results = []
...
class FunctestChecker(Checker):
    ...
    def get_status(self, results_dir, testcase):
        ...
        if os.path.isfile(total_file):
            criteria, sub_testcase = self.get_status_from_total_file(total_file,
                                                                testcase)
            if criteria == 'FAILED':
                return 'FAILED', None
