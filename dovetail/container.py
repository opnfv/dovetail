...
class Container(object):
    ...
    def get_docker_image(cls, type):
        try:
            ...
            cls.logger.exception('There is no key {} in {} '
                                 'config file.'.format(e, type))
            return None
    ...
    def openrc_volume(cls, type):
        ...
        if os.path.isfile(dovetail_config['openrc']):
            ...
            cls.logger.error("File {} doesn't exist.".format(
                dovetail_config['openrc']))
            return None
    ...
    def create(cls, type, testcase_name):
        ...
        try:
            ...
            cls.logger.warn('Failed to get hosts info in {}/hosts.yaml, '
                            'maybe some issues with domain name '
                            'resolution.'.format(hosts_config_path))
        ...
        if cacert is not None:
            if not os.path.isfile(cacert):
                cls.logger.error("Env variable 'OS_CACERT' is set to {} "
                                 "but the file does not exist.".format(cacert))
                return None
            elif not dovetail_config['config_dir'] in cacert:
                cls.logger.error("OS_CACERT file has to be put in {}, which "
                                 "can be mount into container.".format(
    ...
    def pull_image_only(cls, image_name):
        ...
        ret, msg = dt_utils.exec_cmd(cmd, cls.logger)
        if ret != 0:
            cls.logger.error('Failed to pull docker image {}!'.format(
                image_name))
            return False
    ...
    def pull_image(cls, validate_type):
        ...
        if cls.has_pull_latest_image[validate_type] is True:
            cls.logger.debug('{} is already the newest version.'.format(
                docker_image))
            return docker_image
        ...
        if not new_image_id:
            cls.logger.error("Failed to get the id of image {}.".format(
                docker_image))
            return None
        if not old_image_id:
            return docker_image
        if new_image_id == old_image_id:
            cls.logger.debug('Image {} has no changes, no need to '
                             'remove.'.format(docker_image))
        else:
