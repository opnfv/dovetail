...
def check_tc_result(testcase, logger):
    ...
    if dt_cfg.dovetail_config['report_dest'] == "file":
        ...
        if os.path.isfile(result_file):
            logger.info("Results have been stored with "
                        "file {}.".format(result_file))
        else:
            logger.error("Failed to store results with "
                         "file {}.".format(result_file))
...
def main(*args, **kwargs):
    ...
    if testsuite_validation and testarea_validation:
        ...
        logger.error('Invalid input commands, testsuite {} '
                     'testarea {}'.format(kwargs['testsuite'], testarea))
