...
class OpenStackUtilsTesting(unittest.TestCase):
    ...
    def test_exec_log_logger_other(self):
        logger = Mock()

        dovetail_utils.exec_log(verbose=True, logger=logger, msg='',
                                level='other', flush=True)
    ...
    def test_hosts_info_no_ip(self, mock_python_h_entry, mock_python_hosts):
    ...
    @patch('python_hosts.Hosts', autospec=True)
    @patch('python_hosts.HostsEntry', autospec=True)
    def test_hosts_info_no_hosts(self, mock_python_h_entry, mock_python_hosts):
    ...
    @patch('python_hosts.Hosts', autospec=True)
    @patch('python_hosts.HostsEntry', autospec=True)
    def test_hosts_info(self, mock_python_h_entry, mock_python_hosts):
        ip = '127.0.0.1'
        hostnames = ['host_one']
        hosts_obj = Mock()
        entry_obj = Mock()
