...
monkeypatch.setattr(click, 'option', Mock())
monkeypatch.setattr(click, 'argument', Mock())
monkeypatch.setattr(click, 'command', Mock())
o_value = {'key': {'flags': [], 'path': None}}
a_value = {'key': {'flags': [], 'path': None}}
conf_dict = {
    'cli': {'options': {'key': o_value}, 'arguments': {'key': a_value}}
}
monkeypatch.setattr(DovetailConfig, 'dovetail_config', conf_dict)
...
class RunTesting(unittest.TestCase):
    ...
    def test_main(self, mock_create_logs, mock_run, mock_get_list, mock_check,
        ...
        kwargs_dict = {
            'testsuite': 'testsuite', 'debug': True, 'report': 'report'}
        args_list = []

        dt_run.main(*args_list, **kwargs_dict)
