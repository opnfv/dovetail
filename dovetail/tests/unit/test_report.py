...
class ReportTesting(unittest.TestCase):
    ...
    def test_functest_rally_crawler_crawl(self, mock_path, mock_utils,
        ...
        mock_json.loads.return_value = data_dict
        mock_json.load.return_value = rally_dict
        mock_utils.get_duration.return_value = 'duration'
    ...
    def test_functest_rally_crawler_crawl_invalid(self, mock_load, mock_open):
        logger_obj = Mock()
        dt_report.FunctestRallyCrawler.logger = logger_obj
        file_path = 'file_path'
        testcase_obj = Mock()
        file_a = Mock()
        mock_open.return_value.__enter__.return_value = file_a
        ...
        mock_open.assert_called_once_with(file_path, 'r')
        mock_load.assert_called_once_with(file_a)
        self.assertEquals({}, result)

    @patch('__builtin__.open')
    @patch('dovetail.report.json.load')
    def test_functest_rally_crawler_crawl_error(self, mock_load, mock_open):
        logger_obj = Mock()
        dt_report.FunctestRallyCrawler.logger = logger_obj
        file_path = 'file_path'
        testcase_obj = Mock()
