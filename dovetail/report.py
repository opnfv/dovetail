...
class Report:
    ...
    def generate_json(cls, testsuite_yaml, testarea, duration):
        ...
        report_obj['upload_date'] = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
    ...
    def generate(cls, testsuite_yaml, testarea, duration):
        ...
        if report_data['duration'] == 0:
            ...
            report_txt += 'Duration: %.2f seconds\n\n' % report_data['duration']
        ...
        testcase_num = {}
        testcase_passnum = {}
        for area in dt_config.testarea_supported:
            sub_report[area] = ''
            testcase_num[area] = 0
            testcase_passnum[area] = 0

        spec_link = 'https://github.com/opnfv/dovetail/tree/master/dovetail/testcase'
        for testcase in report_data['testcases_list']:
            pattern = re.compile('|'.join(dt_config.testarea_supported))
            area=pattern.findall(testcase['name'])[0]
            sub_report[area] += '- <%s> %s result: <%s>\n' % (spec_link, testcase['name'], dt_config.dovetail_config['result_dir'])
            testcase_num[area] +=1
            total_num +=1
        ...
        for key in sub_report:
            if testcase_num[key] != 0:
                ...
                doc_link = 'https://github.com/opnfv/dovetail/tree/master/docs/testsuites/%s' % key
