...
from encrypt import AESCipher, RSACipher
        ...
        try:
            if not encrypt:
                ...
                cls.logger.info('AES key has been encrypted into file %s.',
                                enc_cfg['encrypt_aes_key'])
            if sign:
                sign_cfg = dt_cfg.dovetail_config['signature']
                cmd = ("./signature sign '%s' '%s'" %
                       (report_txt, sign_cfg['sign_digest']))
                dt_utils.exec_cmd(cmd, cls.logger, exec_msg_on=False)
