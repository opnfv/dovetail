...
def update_envs(options):
    for item in options:
        if item != 'scenario' and options[item] is not None:
            key = cmd_name_trans(item)
            os.environ[key] = options[item]
    update_config_envs('functest')
    update_config_envs('yardstick')


def update_config_envs(type):
    for item in dovetail_config['cli']:
        key = cmd_name_trans(item)
        if key in os.environ.keys():
            old_value = re.findall(r'\s+%s=(.*?)(\s+|$)' % key,
                                   dovetail_config[type]['envs'])
            if old_value == []:
                dovetail_config[type]['envs'] += \
                    ' -e ' + key + '=' + os.environ[key]
            else:
                dovetail_config[type]['envs'] = \
                    dovetail_config[type]['envs'].replace(old_value[0][0],
                                                          os.environ[key])
