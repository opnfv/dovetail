    ...
    for testcase_name in scenario['testcase_list']:
        ...
        if run_test:
            ...
            if testcase.script_type() == 'functest':
                ...
                dt_utils.exec_cmd(cmd,logger,exit_on_error=False)
                sub_cmd = 'python /home/opnfv/repos/functest/ci/run_tests.py -t %s -r' % testcase.script_testcase()
                cmd = 'sudo docker exec %s %s' % (container_id, sub_cmd)
                dt_utils.exec_cmd(cmd,logger, exit_on_error = False)
            else:
                ...
                if container_config[testcase.script_type()]['has_build_images'] == True:
                    cmd = 'sudo docker exec %s %s/run_test.sh %s.yaml %s.out' \
                          % (container_id, container_config[testcase.script_type()]['shell_dir'], testcase.script_testcase(), testcase_name)
                else:
                    container_config[testcase.script_type()]['has_build_images'] = True
                    cmd = 'sudo docker exec %s %s/build_run_test.sh %s.yaml %s.out' \
                          % (container_id, container_config[testcase.script_type()]['shell_dir'], testcase.script_testcase(), testcase_name)
                dt_utils.exec_cmd(cmd,logger,exit_on_error = False)
