...
For dovetail tool, the config files are located in directory ``dovetail/dovetail/conf``, which are written
in yaml format. Since functest and yarstick are used as the dovetail tool upstream, their config file should
be configured correctly, which are shown as ``functest_config.yml`` and ``yardstick_config.yml``.

Functest config template syntax
-------------------------------

One example of the functest config file is shown as,
...
Here ``path`` is the path in testcase config files that you can find the value of parameters.
