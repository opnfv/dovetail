...
``dovetail/testcase`` directory, which are defined in yaml format.
...
- Section 'name' distinguishes different test cases used for compliance and
  certification, and should start with ``dovetail.``
...
- Two kinds of 'type' are currently supported, functest and yardstick
- For functest, the 'testcase' represents the testcases in slicing/tier,
  'sub_testcase_list' represents the testcases in this slicing compliance and
  certification will use. For yardstick, since it is not sliced by now,
  'sub_testcase_list' is not needed, only to edit the 'testcase' part
