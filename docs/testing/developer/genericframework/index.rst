...
- **TestRunnerFactory**: Similar to `TestcaseFactory`, each project has its own
  ...
  and `ShellRunner`. For the projects based on Docker, it needs to create
...
- **Adding test cases that not belong to integrated projects**: The test cases
  ...
  The projects could be in OPNFV, ONAP or other communities. This scenario is a
...
This is the complete format of test case configuration file. Below you can find
...
- **validate**: This is the main section to define how to run this test case.
  ...
  - **pre_condition**: [optional] A list of all preparations needed by this
    ...
    may be offline environments in private labs.
...
only supports integration of projects by using their Docker images. If you design
...
This is the complete format of project configuration file. Below you can find
detailed description for each of the configuration options.
...
If you apply acceptance check then a patch is not going to be verified without
100% coverage.
