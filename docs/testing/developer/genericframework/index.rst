...
- **TestcaseFactory**: For each project, there needs to create its own
  ...
  other special requirements, it only needs to initialize these classes with
...
- **TestRunnerFactory**: Similar to `TestcaseFactory`, each project has its own
  ...
  and `ShellRunner`. For the projects that based on Docker, it needs to create
  their own test runner classes such as `FunctestRunner` which inherit from class
  `DockerRunner`. For other projects that based on Shell, it can use `ShellRunner`
  directly. Test case runners provide many functions to support to run test cases
...
- **Test case groups**: Each group is composed of one project configuration file
  and a set of test cases belonging to this project. These groups are as the input
  of test runners to provide the information of projects and test cases. For
...
- **CrawlerFactory**: This is used to parse the results of test cases and record
  them with unify format. The original result data report by each project is
...
- **CheckerFactory**: This is used to check the result data generated by crawler.
  Each projects should has it own checker class due to the different requirements
