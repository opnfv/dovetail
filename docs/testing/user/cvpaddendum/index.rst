...
this point. E.g., the deployment tools for the SUT and CI/CD toolchain are out
of scope. Similarly, performance benchmarking related testing is also out of scope or
...
presently to derive a working scope for the initial CVP, and lastly projecting what the
community should strive to achieve in future releases.

The overall requirements for CVP can be cateorized simply by the basic cloud
capabilities representing common operations needed by basic VNFs, and additional
requirements for NFV that go beyond the common cloud capabilities including
...
For some of the longer term requirements,
...
we are not yet ready to support these areas at this time, but can start
...
in the following enumerated sections:
...
gateway. The basic VNF has a set of common requirements, including:
  - VNF image management (Refstack)
  - identity management (Refstack)
  - virtual compute (Refstack)
  - virtual storage (Refstack)
  - virtual networks (Refstack)
  - forwarding packets through virtual networks in data path
  - filtering packets based on security rules and port security
...
packet filtering by security group rules and port security, more dynamic use of
...
a common standard to enable portability across compliant platforms.

2. NFV specific funtional requirements
...
common and basic to the OPNFV specific functional extensions.
...
4. Resiliency with load

Resiliency testing with load involves stressing the SUT and verifying its ability
to absorb errors while continuously providing an acceptable level of service.
It is another aspect of the system requirement
...
5. Supporting Security

Security is among the top priorities as a carrier grade requirement by the
end-users. Some of the basic common functions, including virtual network isolations,
security groups, port security and role based access control are already covered as
part of the common capabilities that are verified in CVP.
...
Therefore, we do not add any additional security test cases at this time.

Next, we look at another category of testing using common NFV use cases.
...
Examples such as vIMS (part of Functest), or those which are not yet available
...
7. Future CVP scope enhancements
