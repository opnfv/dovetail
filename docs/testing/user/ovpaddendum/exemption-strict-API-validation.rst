...
the industry, thereby facilitating industry alignment.
...
#. The test result data created by the Dovetail tool includes an explicit
   print-out stating if strict API validation was disabled during the test run
   or not.
...
#. All compliance badges remain valid even after the OVP exemption phase has
   been cancelled.
