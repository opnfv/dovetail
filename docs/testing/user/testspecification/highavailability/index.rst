...
availability on part of OpenStack controller services("nova-api",
"neutron-server", "keystone", "glance-api", "cinder-api") and on "load balancer"
service.

The tests in this test area will evaluate high availability by killing the
processes of above target services, stressing the CPU usage and blocking
disk I/O on the selected controller node, and then check if those
...
SUT is assumed to be in high availability configuration, which means at least
one controller node is in the system under test.

The processes of OpenStack controller services("nova-api", "neutron-server",
"keystone", "glance-api", "cinder-api") and load balancer on controller node
can automatically recover when the failures happen.
...
Preconditions of each test case will be introduced in the following test
descriptions.
...
OpenStack controller nodes. It kills the processes of OpenStack "nova-api"
service on the selected controller node, then checks whether the "nova-api"
service is still available by executing command "openstack server list" and
...
There is at least one controller node, which is providing the "nova-api"
...
* Test action 3: Connect to Node1 through SSH, and then kill the "nova-api"
  processes
...
* Test action 6: Check the status of the "nova-api" process on Node1, and
  restart the process if it is not running.

Pass / fail criteria
''''''''''''''''''''

The killed "nova-api" processes are able to recover within 20s and the service
...
the "neutron-server" service is still available by executing command
"openstack router list" and whether the killed processes are recovered.

Test preconditions
------------------

There is at least one controller node, which is providing the "neutron-server"
service for API end-point.
Denoted as Node1 in the following configuration.
...
The killed "neutron-server" processes are able to recover within 20s and the
service outage time is shorter than 5s.
...
There is at least one controller node, which is providing the "keystone"
service for API end-point.
Denoted as Node1 in the following configuration.
...
* Test action 5: The test passes if process outage time is less than 20s and
  service outage time is less than 5s
* Test action 6: Check the status of the "keystone" process on Node1, and
  restart the process if it is not running for next test cases

Pass / fail criteria
''''''''''''''''''''

The killed "keystone" processes are able to recover within 20s and the service
outage time is shorter than 5s.
...
There is at least one controller node, which is providing the "glance-api"
service for API end-point.
Denoted as Node1 in the following configuration.
...
The killed "glance-api" processes are able to recover within 20s and the
service outage time is shorter than 5s.
...
The killed "cinder-api" processes are able to recover within 20s and the
service outage time is shorter than 5s.
...
The service outage time is shorter than 5s.
...
the specific disk is blocked, which breaks down the OpenStack services on this
...
* Test action 1: Connect to Node1 through SSH, and then block disk I/O by
...
* Test action 7: Release the blocked disk I/O and delete the created "test-001"
  flavor

Pass / fail criteria
''''''''''''''''''''

The flavor "test-001" is created successfully and the service outage time is
shorter than 5s.
...
There is at least one controller node, which is providing the "load balancer"
...
* Test action 5: The test passes if process outage time is less than 20s and
  service outage time is less than 5s
* Test action 6: Check the status of processes of "load balancer" service on
  Node1, and restart those processes if they are not running for next test
  cases.

Pass / fail criteria
''''''''''''''''''''

The killed processes of "load balancer" service are able to recover within 20s
and the service outage time is shorter than 5s.
