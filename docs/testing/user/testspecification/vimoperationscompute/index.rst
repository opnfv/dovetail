...
* Test action 2: Create a new server image IMG1 from VM1, specifying image name
  and image metadata. Wait for IMG1 to reach 'ACTIVE' status, then delete IMG1
* **Test assertion 1:** Verify IMG1 is created with correct image name and image
  metadata; verify minimum RAM is the same as the VM1 original image and minimum
  disk is the same as the VM1 original image or flavor size
* **Test assertion 2:** Verify IMG1 is deleted correctly
* Test action 3: Create another server IMG2 from VM1, specifying image name
  with a 3 byte utf-8 character
* **Test assertion 3:** Verify IMG2 is created correctly
...
Test execution
''''''''''''''
* Test action 1: Get the action details ACT_DTL of a provided server VM1
* **Test assertion 1:** Verify ACT_DTL's 'instance_uuid' matches VM1's ID and
  ACT_DTL's 'action' matched 'create'
* Test action 2: Create a server VM2 and wait for VM2 to reach 'ACTIVE' status
* Test action 3: Delete server VM2 and wait for VM2 to reach termination
* Test action 4: Get the action list ACT_LST of VM2
...
* **Test assertion 1:** Verify VM1 is created with KEYP1
...
- Create a server with admin password/a name that already exists/numeric name/a really
  long metadata/name length exceeding 255 characters/an unknown flavor/an unknown image ID/
  an invalid network UUID
- Delete a server using a server ID that exceeds length limit/a negative server ID
- Get a non existent server details
- Verify the instance host name is the same as the server name
- Create a server with an invalid access IPv6 address
- List all existent servers
- Filter the (detailed) list of servers by flavor/image/server name/server status/limit
- Lock a server and try server stop/unlock/retry
- Get/delete metadata from a server
- List/set metadata for a servers
- Reboot/rebuild/stop/start a server
- Update a server's access addresses/server name
...
* **Test assertion 1:** Verify the password is set correctly in the returned response
...
* Test action 12: Get a non existent server by using a random generated server ID
...
* Test action 38: List all existent servers and filter the server list by a non existing flavor
* **Test assertion 37:** Verify the filtered server list is empty
* Test action 39: List all existent servers and filter the server list by a non existing image
* **Test assertion 38:** Verify the filtered server list is empty
* Test action 40: List all existent servers and filter the server list by a non existing server name
...
* Test action 42: List all existent servers and filter the server list by a non existing server status
...
* Test action 56: Soft reboot a non existent server
...
* Test action 61: Rebuild a non existent server
...
* Test action 69: Update the server name of a non existent server
...
* Delete a server with a server ID that exceeds the length limit or a non existent server ID is not allowed
...
* Filter the (detailed) list of servers by flavor/image/server name/server status/display limit
* Filter the list of servers by a future date
* Filter the list of servers by an invalid date format/a negative display limit/a string type
  display limit value is not allowed
* Filter the list of servers by a non existent flavor/image/server name/server status is not allowed
* Deleted servers are not in the list of servers
* Deleted servers do not show by default in list of servers
* Locked server is not allowed to be stopped by non-admin user
* Can get/delete metadata from servers
* Can list/set/update server metadata
* Create a server with name parameter empty is not allowed
* Hard reboot a server and the server should be power cycled
* Reboot/rebuild/stop a non existent server is not allowed
* Rebuild a server using the provided image and metadata
* Stop and restart a server
* A server's name/access addresses can be updated
* Update the name of a non existent server is not allowed
