...
* Test action 2: Create a new server image IMG1 from VM1, specifying image name
  and image metadata. Wait for IMG1 to reach 'ACTIVE' status, and then delete IMG1
...
- List/set metadata for a server
...
* Filter the list of servers by a nonexistent flavor/image/server name/server status is not allowed
...
* Reboot/rebuild/stop a nonexistent server is not allowed
