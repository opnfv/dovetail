...
purposes or their combinations. According to the testing goal stated in
the test scope section, preceding test will not affect the subsequent test
as long as the SUT is able to sustain the given stress while providing
...
VM1 in a VM pair pings VM2 for *P1* times with *P1* packets. The connectivity could be validated iff
VM1 successfully pings VM2 with these *P1* packets.
Subtest 1 is finished iff all the concurrent (N1) requsts for creating VM pairs are fulfilled with
returned values that indicate the statuses of the VM pairs creations.
...
is smaller than *N3*. Subtest 2 will not be excuted and SUT will be marked with FAIL.
...
The overall resiliency testing workflow will not proceed if the SUT is marked with FAIL.
That is, subsequent test cases do not need to be executed any more.
