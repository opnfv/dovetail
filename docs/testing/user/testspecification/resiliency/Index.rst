...
purposes or their combinations. These test cases are intended to be executed
with the beginning test cases validating basic capabilities and
latter test cases validating composed capabilities of the SUT.
...
(through different OpenStack related components) and providing acceptable capacity under stressful conditions.
...
Subtest 1 is finished iff all the concurrent (N1) requests for creating VM pairs are fulfilled with
...
The whole resiliency testing workflow will not proceed if the SUT is marked with FAIL.
