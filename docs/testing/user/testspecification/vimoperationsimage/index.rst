...
VIM image operations. The tests in this area will evaluate image deletion tests,
image get tests, CRUD image operations, image list tests and image update tests
using the Glance v2 API.
...
The system under test is assumed to be the NFVi and VIM in operation on an
...
* **Test assertion 3:** Use the deleted image's id to show it get a 404 reponse code.
  Use the deleted image's id to delete it get a 404 reponse code.
...
* Test action 1: Create 6 images and store their ids in a created images list.
  List all images and check whether the ids listed are in the created images list.
...
* Test action 1: Create 6 images with a random size ranging from 1024 to 4096 and
  visibility 'private', and set their (container_format, disk_format) pair to be
  (ami, ami), (ami, ari), (ami, aki), (ami, vhd), (ami, vmdk) and (ami, raw).
  Store their ids in a list. Upload the binary images data.
* Test action 2: Use Glance v2 API to list all images whose container_format is 'ami'
  and store the response details in a list.
* **Test assertion 1:** The list is not empty and all the value of container_format
  in the list are 'ami'.
* Test action 3: Use Glance v2 API to list all images whose disk_format is 'raw'
  and store the response details in a list.
* **Test assertion 2:** The list is not empty and all the value of disk_format
  in the list are 'raw'.
* Test action 4: Use Glance v2 API to list one image by setting limit to be 1.
...
* Test action 8: Use Glance v2 API to list the images whose visibility is private and
  store the response details in a list.
* **Test assertion 7:** All visibility of images in the list are private.
