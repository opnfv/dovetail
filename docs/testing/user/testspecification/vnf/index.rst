...
For the execution of this test scenario the following components are needed:
 - VNF Manager (juju)
 - Test executor (ABot), test Orchestration system that enables test scenarios to be defined in
   high-level DSL
 - VMs which are act as VNFs (including the VNF that is the SUT for test case)
...
* Test action 4: ABot test results are parsed accordingly and pushed to
  Functest db.
...
This test verify that the vEPC support most of the essential functions defined
by 3GPP technical specs.

The data have been reported by the test case and stored in the functest db
could be used as input and the results could be compared with the 3GPP specs.
...
The deployed VMs are deleted running clean-up operations.
