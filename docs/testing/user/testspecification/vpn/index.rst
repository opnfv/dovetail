...
This test area assumes support for the following specifications:
...
The system under test is assumed to be the NFVi in operation on a Pharos
...
The test area is structured as four sequential tests with inherent dependencies
between each test in the test area.  The test area evaluates the ability of the
SUT to establish connectivity between Virtual Machines using an appropriate
route target configuration, reconfigure the route targets to remove
connectivity between the VMs, then reestablish connectivity by re-association.
...
This test evaluates the use case where an NFVi tenant wants to use a BGPVPN to
...
* **Test action 1:** VM1 connects to VM2 via SSH. The test passes if VM1 can successfully connect to VM2 via SSH.
* **Test action 2:** VM1 connects to VM3 via SSH. The test passes if VM1 can successfully connect to VM3 via SSH.

* Create VPN2 with iRT=eRT=RT2 and associate N2 to it
* **Test action 3:** VM4 connects to VM5 via SSH. The test passes if VM4 can successfully conncet to VM5 via SSH.
* **Test action 4:** VM4 connects to VM1 via SSH. The test passes if VM4 cannot execute the command "hostname" on VM1 via SSH.


Pass / fail criteria
''''''''''''''''''''

This test evaluates the virtual machine's ability to connect effectively with each other using only the described IP address schema. Specifically, the test verifies that:

* Adjusting the ingress and egress route targets should prohibit and allow routing as described in the test actions.
* The test will fail unless overlapping IP addresses are able to be used in different VPN's.
* The test will fail unless the correct hostname be returned by the target virtual machine.
* The test will fail if for any reason IP routing is not able to be achieved between the virtual machines.
