...
that will initiate send pings by using a custom userdata script provided by nova metadata service.
...
* Test action 1: create a private tenant network by using neutron client, one subnet and one
  router are created in the network by using neutron client, one interface is added between
  the subnet and router, one gateway is added to the router by using neutron client,
...
* Test action 2: create an security group by using neutron client, storing the security group
