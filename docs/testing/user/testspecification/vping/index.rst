...
requires L2 connectivity between those VMs. Target IP is passed to the VM
sending pings by using a custom userdata script provided by nova metadata service.
...
* Test action 6: inside VM2, the ping script is executed automatically when boot and it contains an infinite
  loop doing the ping until the return code is 0 or timeout reached, for each ping, when the return code is 0,
  "vPing OK" is printed in the VM2 console-log, otherwise, "vPing KO" is printed.
...
L2 connectivity between those VMs, an SSH connection is establised from the host to
a floating IP associates to VM2 and executes ``ping`` on VM2 with the IP of VM1 as target.
...
* Test action 7: establish SSH connection between host or jumphost and VM2 through the floating IP
* Test action 8: copy the Ping script from host or jumphost to VM2 using SCPClient
* **Test assertion 7:** the Ping script can be found inside VM2
* Test action 9: inside VM2, to execute the Ping script to ping VM1, the Ping script contains an infinite
  loop doing the ping until the return code is 0 or timeout reached, for each ping, when the return code is 0,
  "vPing OK" is printed in the VM2 console-log, otherwise, "vPing KO" is printed.
  Monitoring the console-log of VM2 to see the response generated by the script.
* **Test assertion 8:** "vPing OK" is detected, when monitoring the console-log in VM2
