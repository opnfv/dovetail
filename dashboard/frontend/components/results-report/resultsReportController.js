...
(function () {
    ...
    ResultsReportController.$inject = [
        ...
        '$uibModal', 'dovetailApiUrl', 'raiseAlert'
    ];

    /**
     * Dovetail Results Report Controller
    ...
    function ResultsReportController($http, $stateParams, $window,
        ...
        ctrl.updateTestcases = updateTestcases;
        ...
        /** The target OPNFV to compare against. */
        ctrl.target = 'platform';

        /** Mappings of Dovetail components*/
        ctrl.targetMappings = {
            'platform': 'System Under Test'
        };

        /** The schema version of the currently selected testcases data. */
        ...
        /**
         * Retrieve an array of available testcases files from the Dovetail
        ...
        function getVersionList() {
            ...
            ctrl.versionsRequest =
                $http.get(content_url).success(function (data) {
                    ...
                    if (!ctrl.version) {
                        // Default to the first approved testcase which is
                        // expected to be at index 1.
                        ctrl.version = ctrl.versionList[1];
                    }
                    ctrl.updateTestcases();
        ...
        /**
         * Retrieve results from the Dovetail API server based on the test
         ...
         */
        function getResults() {
            var content_url = dovetailApiUrl + '/results/' + ctrl.testId;
            ctrl.resultsRequest =
                $http.get(content_url).success(function (data) {
                    ctrl.resultsData = data;
                    ctrl.version = ctrl.resultsData.meta.testcase;
        ...
        function shareTestRun(shareState) {
            var content_url = [
                dovetailApiUrl, '/results/', ctrl.testId, '/meta/shared'
            ].join('');
            if (shareState) {
                ctrl.shareRequest =
        ...
        function deleteTestRun() {
            var content_url = [
                dovetailApiUrl, '/results/', ctrl.testId
            ].join('');
            ctrl.deleteRequest =
                $http.delete(content_url).success(function () {
                    $window.history.back();
        ...
        function updateTestcases() {
            ctrl.testcaseData = null;
            ctrl.showError = false;
            var content_url = dovetailApiUrl + '/testcases/' +
                ctrl.version;
            ctrl.capsRequest =
                $http.get(content_url).success(function (data) {
                    ctrl.testcaseData = data;
                    ctrl.schemaVersion = data.schema;
                    ctrl.buildCapabilitiesObject();
                }).error(function (error) {
                    ctrl.showError = true;
                    ctrl.testcaseData = null;
                    ctrl.error = 'Error retrieving testcase date: ' +
                        angular.toJson(error);
                });
        }
        ...
        function getTargetCapabilities() {
            var components = ctrl.testcaseData.components;
            var targetCaps = {};
            ...
            if (ctrl.target === 'platform') {
                var platform_components =
                    ctrl.testcaseData.platform.ComplianceSet;
        ...
        function buildCapabilityV1_2(capId) {
            ...
            var capDetails = ctrl.testcaseData.capabilities[capId];
        ...
        function buildCapabilityV1_3(capId) {
            ...
            // Loop through each test belonging to the capability.
            angular.forEach(ctrl.testcaseData.capabilities[capId].tests,
         ...
         * capability data retrieved from the Dovetail API server.
         */
        function buildCapabilitiesObject() {
            ...
            ctrl.caps = {
                'ComplianceSet': {'caps': [], 'count': 0, 'passedCount': 0,
                        'flagFailCount': 0, 'flagPassCount': 0},
                'ProposedTests': {'caps': [], 'count': 0, 'passedCount': 0,
                        'flagFailCount': 0, 'flagPassCount': 0},
            };
            ...
            switch (ctrl.schemaVersion) {
                ...
                default:
                    ctrl.showError = true;
                    ctrl.testcaseData = null;
                    ctrl.error = 'The schema version for the testcase ' +
            ...
            ctrl.ComplianceSetPassPercent = (ctrl.caps.ComplianceSet.passedCount *
                100 / ctrl.caps.ComplianceSet.count);

            ctrl.totalComplianceSetFailCount = ctrl.caps.ComplianceSet.count -
                ctrl.caps.ComplianceSet.passedCount;
            ctrl.totalComplianceSetFlagCount =
                ctrl.caps.ComplianceSet.flagFailCount +
                ctrl.caps.ComplianceSet.flagPassCount;
            ctrl.totalNonFlagCount = ctrl.caps.ComplianceSet.count -
                ctrl.totalComplianceSetFlagCount;
            ctrl.nonFlagPassCount = ctrl.totalNonFlagCount -
                (ctrl.totalComplianceSetFailCount -
                 ctrl.caps.ComplianceSet.flagFailCount);

            ctrl.nonFlagComplianceSetPassPercent = (ctrl.nonFlagPassCount *
        ...
        function getFlaggedReason(test, capObj) {
            if ((ctrl.schemaVersion === '1.2') &&
                ...
                return 'Dovetail has flagged this test.';
            }
